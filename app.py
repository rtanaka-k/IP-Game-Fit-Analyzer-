...
まず頭の中で「このIPをゲームにしたとき、プレイヤーが最も興奮する瞬間は何か」を考えてから、
その体験を最大化できるジャンルを選んでください。スコアの高低から機械的に選ばないこと。
ジャンルDBは参考にしてよいですが、DBに縛られず複合ジャンルや独自定義も使ってください。

推薦は以下の3カテゴリーで行ってください：

1. **本命（Most Likely）** × 2案：そのIPならではの体験を最大化できる、実現性の高い提案
2. **対抗（Dark Horse）** × 1案：一見意外だが、IPの隠れた魅力を引き出せる提案
3. **大穴（Wild Card）** × 1案：前例がないが、ハマれば新ジャンルを生み出しうる提案

各提案に必ず以下を含めること：
- **ジャンル**
- **なぜこの組み合わせか**：プレイヤー体験との接続を明示しながら（3文）
- **30秒ピッチ**：「このゲームは〇〇するゲームです。プレイヤーは〜〜の体験ができます」形式で、開発会社への持ち込み口頭説明文として（3〜4文）
- **タイトルコピー案**：キャッチコピー1行（例：「死ぬたびに、強くなれ。」）
- **類似成功事例**：あれば
- **最大のリスク**：1文で本質的な懸念のみ
- **プラットフォーム／収益モデル**
